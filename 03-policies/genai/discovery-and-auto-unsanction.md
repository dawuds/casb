...
### Use case 1 — Tier-1 ASEAN universal bank, post-Storm-2372 consumer-AI clamp
...
- **Scope:** all employees + contractors on managed endpoints (~28k devices); MDA Shadow IT discovery via MDE feed; auto-unsanction on the LLM and AI-Coding-Assistant sub-categories at risk score ≤ 5; allowlist = ChatGPT Enterprise (tenant SSO'd), M365 Copilot, internal AI gateway
...
- **Scope:** Engineering + QA + Product BUs (~800 users); MDA discovery + MDE NP block on AI-Coding-Assistant sub-category specifically (not general LLM); allowlist = GitHub Copilot Enterprise on CDE-isolated tenant only; secondary control = pre-commit hook scrubbing PAN-format from any AI-bound payload
...
- **Scope:** joint workstream — MDA OAuth apps page (Policy 2a) + App Governance Predictive Risk (Policy 2b) + GenAI auto-unsanction (this policy). Approved-AI vendors must pass BOTH the network-discovery allowlist AND the OAuth-grant verified-publisher restriction in Entra
...
| W2 | Pull 30-day GenAI baseline from Cloud Discovery: app list by sub-category (LLM / AI Coding Assistant / Image / Video / Voice-Avatar); user count per app; traffic-volume rank | Baseline inventory; named long-tail vs head-of-curve apps |
...
| W5 | Policy in **alert-only** mode: App discovery policy with category filter = LLM + AI Coding Assistant sub-categories, risk score ≤ 5, users/day > 10, action = Alert (NOT Tag as Unsanctioned yet) | Alert volume measured; FP candidates identified |
...
| W9 | Broaden category filter from LLM-only to all GenAI sub-categories (Image, Video, Voice-Avatar) as appropriate per the firm's posture; tighten risk-score threshold or users-per-day threshold based on W7-W8 data | Full GenAI sub-category coverage live |
...
- **Apps:** GenAI sub-categories (LLM / AI Coding Assistant / AI Image Generator / AI Video Generator / AI Voice-Avatar) — the 2025 H2 sub-category split, NOT the monolithic "Generative AI" category
...
```yaml
policy:
  name: "GenAI auto-unsanction — LLM + AI Coding Assistant"
  type: AppDiscoveryPolicy
  category_filter:
    include:
      - "LLM"                                # 2025 H2 sub-category
      - "AI Coding Assistant"
      # Image / Video / Voice-Avatar added in phase 2 per the firm's posture
    exclude: []
  risk_filter:
    risk_score_max: 5                        # lower = higher risk; 1-10 MS scale
  usage_filter:
  ...
  endpoint_propagation:
    ...
    propagation_window_minutes: 60           # observed; not Microsoft-published SLA
  alerts:
    severity: Medium
...
The `risk_score_max: 5` is the MDA-published threshold heuristic; the operationally-important guard is the `app_allowlist` + the **parallel internal risk-acceptance workflow** — the auditor will probe whether the firm relies on Microsoft's opaque score or on a documented internal decision. Document both.

## Variants

### Industry-specific

- **BFSI:** the dominant scope. Allowlist tightly curated; consumer-tier consumer AI universally blocked. The hard fight is between the engineering org wanting Copilot / Cursor / Replit Ghostwriter and the risk team wanting only the enterprise-tier with SSO + DPA. AML.T0010 supply-chain risk on the allowlist is named on the risk register. BNM RMiT third-party / outsourcing clauses apply to allowlisted vendors [VERIFY against current edition].
- **Healthcare:** PHI exposure is the dominant fear. AI vendors must pass HIPAA-BAA scrutiny before allowlisting; even "internal-data-OK" tier is suspect because internal data includes PHI. Image-generator sub-category often universally blocked because of PHI-in-radiology concern.
...
- **Advanced:** AI-vendor risk-assessment template integrated with the wider vendor-risk programme (not a separate workflow); Purview Adaptive Protection ties AI usage signal to user-risk score; Microsoft Edge for Business `gen-ai.app-control` overlays browser-layer enforcement on Edge-on-Windows estates; GSA-native GenAI-category block for off-corp roaming; DSPM-for-AI prompt-capture deployed on the allowlisted enterprise-AI vendors (deep capture on ChatGPT Enterprise / Copilot, shallow on Claude / Gemini per the MDA playbook DSPM table). Quarterly **AML.T0010 supply-chain rehearsal** — what does the firm do if an allowlisted AI vendor announces a compromise.

## Control mappings

- BNM RMiT clause(s): [BNM RMiT third-party risk + DLP + AI governance overlay](../../06-compliance/malaysia/bnm-rmit.md) `[VERIFY against current edition]`
...
- MITRE ATLAS coverage: `AML.T0010` (ML Supply Chain Compromise) — partial via allowlist governance; `AML.T0024` (Exfiltration via ML Inference API) — partial via category block
...
| Marketing-team use of AI image generators (DALL-E web, Midjourney, Adobe Firefly) — legitimate non-sensitive use case | Sub-category-specific allowlist tier (Public-OK tier); typically allowlist Adobe Firefly enterprise; block Midjourney free / DALL-E web by individual workflow |
...
- **Exception-handling load:** medium — 5-15 intake-ticket requests per week typical at a tier-2 BFSI tenant; quarterly allowlist review consumes ~2 days of cross-functional time (Risk + Legal + IT + business owner)
- **Triage load:** low — block at endpoint is silent (no user-side ticket unless they raise via intake); SOC alert load is low because endpoint blocks generate aggregated rather than per-event alerts; ~5-20 alerts per week per 1k users at maturity
...
- DSPM-for-AI prompt capture (separate policy, see MDA Policy 9) is **high-intrusion processing** under GDPR Art. 35 — DPIA before pilot; truncated content snippets in audit log are themselves regulated content
...
- **Audit cycle:** allowlist + intake-process evidence + quarterly-review minutes feed BNM RMiT third-party / outsourcing attestation [VERIFY]; ISO 42001 AI-management-system controls if certified `[VERIFY]`; NIST CSF 2.0 GV.SC + ID.AM-04 evidence pack.
- **Board reporting:** quarterly metric — (a) allowlist size + change-rate, (b) blocked-attempt count + trend, (c) intake-ticket SLA performance, (d) AML.T0010 supply-chain incidents on allowlisted vendors. The trend matters more than the absolute number.
...
- **OAuth governance interlock:** AI-via-OAuth apps bypass the network-egress discovery (per Use case 4); approved-AI vendors must pass both the network-discovery allowlist AND the OAuth-grant verified-publisher restriction in Entra. Document the interlock; do not let the two workstreams diverge.
- **DSPM-for-AI integration:** prompt-body capture on allowlisted enterprise-AI vendors feeds Purview Insider Risk Management adaptive scoping (per MDA Policy 12); deep capture only on ChatGPT Enterprise + Copilot per the MDA playbook DSPM table; shallow on Claude / Gemini.
...
3. **"Rely on Microsoft's opaque risk score as sole basis for automated enforcement"** — auditor cannot accept it; the "90+ factors" are uncited and weights not disclosed. Document a parallel internal risk-acceptance / appeal process that decides which vendors land in which sanction tier independent of the catalogue score
4. **"Use the monolithic Generative AI category filter"** — lumps LLMs with image / video / voice tools, which have different risk classes. Use the 2025 H2 sub-category split (LLM / AI Coding Assistant / AI Image Generator / AI Video Generator / AI Voice-Avatar)
...
- 2026 alternatives: Microsoft Edge for Business `gen-ai.app-control` (H2 2026 GA — verify) + GSA native GenAI category block supersede MDA → MDE NP chain for Edge-on-Windows estates with GSA licensed; prefer these where deployable
