...
> MDA playbook reference: companion to [internal MDA Policy 9 — clipboard paste block](../../04-vendors/microsoft-defender-for-cloud-apps.md) (internal numbering; not a Microsoft-published reference).
...
The policy is rarely the first GenAI control deployed — Discovery (internal MDA Policy 1) and auto-unsanction (internal MDA Policy 8) usually come first. Tenant pinning is the second-wave control once a sanctioned tenant exists. **Practitioner inference:** the cross-vendor tenant-pinning architecture below (combining Microsoft Tenant Restrictions v2 with vendor-specific SAML / header conventions for non-Microsoft AI vendors) is a practitioner-constructed pattern, not a Microsoft-documented end-to-end architecture. Microsoft documents Tenant Restrictions v2 as an Entra + Global Secure Access control (Identity Zero Trust pillar Objective IV — "Restrict user consent to applications") [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/identity]; the multi-vendor extension to ChatGPT / Claude / Gemini auth surfaces is field practice (not Microsoft-architected). The hardest part is not the technology — `Restrict-Access-To-Tenants` headers are well-documented — it is the cross-vendor consistency: ChatGPT supports SAML tenant-restriction for Enterprise / Team; Gemini for Workspace uses Google's own `X-GoogApps-Allowed-Domains`; Anthropic Claude added SCIM + SAML in 2025; Copilot is implicitly tenant-pinned via Entra. Five vendors, five header conventions, three sign-in surfaces (browser, native desktop, mobile) — every gap is an exfiltration channel.
...
- **Trigger:** Storm-2372 device-code-phishing campaign (Microsoft Threat Intelligence, February 2025) tricked three engineers into consenting to malicious OAuth apps that masqueraded as AI plugins [Microsoft Security Blog: https://www.microsoft.com/en-us/security/blog/2025/02/13/storm-2372-conducts-device-code-phishing-campaign/]. Forensic review surfaced a broader pattern — engineers had also been signing into personal ChatGPT / Claude with personal Google accounts on corporate Macs, sometimes pasting production code. The board treated both as a single class — uncontrolled AI-vendor identity sprawl — and asked for a single answer
...
- **Scope:** Cards + Acquiring + Risk + Customer Service business units (~2.5k users initially) handling CHD; tenant pinning on ChatGPT, Gemini, Claude, Copilot; complementary clipboard-paste block on ChatGPT Enterprise (internal MDA Policy 9) to stop CHD-pattern paste into prompts even on the enterprise variant
...
- **Advanced deployment** looks like: every sanctioned AI vendor in the ICT third-party register with full vendor-risk attestation cycle (SOC 2, ISO 27018, EU AI Act provider obligations where applicable, DORA contract clauses); DSPM-for-AI prompt capture on the sanctioned tenants feeds Purview IRM as a signal — note that the Microsoft-documented capability matrix differs by vendor: ChatGPT Enterprise supports DSPM + Audit + Data classification + IRM + eDiscovery + DLM (NOT DLP and NOT sensitivity labels) [Microsoft Learn: https://learn.microsoft.com/en-us/purview/ai-chatgpt-enterprise]; Claude Enterprise (preview) supports DSPM + Audit ONLY (NOT DLP, sensitivity labels, IRM, eDiscovery) [Microsoft Learn: https://learn.microsoft.com/en-us/purview/ai-claude-enterprise]; Google Gemini is covered via third-party-AI-sites coverage (browser extension + endpoint DLP only); joiner-mover-leaver flow revokes enterprise AI access in <24h; tenant-pinning posture per-jurisdiction (different sanctioned-AI register per region); board cyber report carries "% of GenAI sessions on enterprise tenant" as a steady-state KPI; gap analysis against EU AI Act deployer obligations + ASEAN equivalents documented.

## Control mappings

- CIS Microsoft 365 Foundations Benchmark v5.0.0 (30 April 2025):
  - CIS 5.1.6.1 (L2) — cross-tenant access settings for Microsoft endpoints (the Entra-native half of tenant pinning)
  - CIS 5.2.2.12 (L1) — Conditional Access policy to require approved client apps / Conditional Access App Control on sanctioned tenants
  - CIS 2.4.3 (L2) — umbrella control for Defender for Cloud Apps deployment
- Microsoft Secure Score — Identity group improvement actions (Conditional Access + cross-tenant access settings) [Microsoft Learn: https://learn.microsoft.com/en-us/defender-xdr/microsoft-secure-score] [Microsoft Learn: https://learn.microsoft.com/en-us/defender-xdr/microsoft-secure-score-improvement-actions]
- Microsoft Zero Trust — Identity pillar Objective IV "Restrict user consent to applications" [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/identity]
...
Material on regulatory clauses is illustrative — not legal / regulatory advice.
...
Typical FP-rate trajectory for tenant pinning in a tier-2 BFSI tenant new to enterprise GenAI. Practitioner-observed ranges; not from Microsoft documentation. Validate against tenant baseline.
...
10. **"Allow the break-glass-admin account to bypass tenant pinning permanently"** — break-glass = emergency use only; permanent bypass on a privileged account is itself the attack surface (Storm-0501-class persistence) [Microsoft Threat Intelligence — Microsoft canonical threat-actor naming taxonomy: https://learn.microsoft.com/en-us/unified-secops/microsoft-threat-actor-naming]. Time-box break-glass; alert on use; rotate quarterly
...
- New AI vendors emerging faster than the sanctioned register can keep pace (a new entrant goes from launch to 1k corporate users in weeks) — Shadow IT discovery (internal MDA Policy 1) feeds the sanction-or-block decision but there is a 2-4 week response lag
