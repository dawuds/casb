...
> MDA playbook reference: [Policy 2a](../../04-vendors/microsoft-defender-for-cloud-apps.md) (Classic OAuth apps page anomalies). The "Policy 2a" numbering is internal to this playbook, not Microsoft documentation.
...
**Reframing:** this is a **cleanup control, not a primary defence.** Primary control for T1528 is the Entra admin-consent workflow + verified-publisher restriction, configured upstream in Entra (not MDA). MDA Policy 2a inspects what already happened. Microsoft anchors the primary control at Identity Zero Trust Objective IV "Restrict user consent to applications" [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/identity] and documents the cleanup workflow at [Microsoft Learn: https://learn.microsoft.com/en-us/defender-office-365/detect-and-remediate-illicit-consent-grants]. Verified-publisher restriction is documented at [Microsoft Learn: https://learn.microsoft.com/en-us/entra/identity-platform/publisher-verification-overview].

> **June 2025 anomaly-policy disablement / rename note.** Several built-in anomaly policies were transitioned to a dynamic-threat-detection model in June 2025 and renamed — Activity from suspicious IP addresses, Suspicious inbox forwarding, Suspicious inbox manipulation rules, Activity from anonymous IP addresses, Ransomware activity, Suspicious file access activity by user, **Unusual ISP for an OAuth App (now "OAuth application activity from an unknown ISP")**, Suspicious email deletion activity. The OAuth-app-as-actor signal referenced in some sections of this policy depends on the renamed policy. Source: [Microsoft Learn: https://learn.microsoft.com/en-us/defender-cloud-apps/anomaly-detection-policy] (Important notice).

## What organisations use this for

Most organisations stand this up *after* a consent-phishing incident — not before. The pattern is consistent: an internal IR ticket surfaces a suspicious "DocuSign" or "Adobe Acrobat" lookalike with `Mail.ReadWrite` + `offline_access` granted by 40 users in the last 72 hours; the SOC discovers there is no central inventory of OAuth grants and no workflow to revoke at scale; the OAuth-apps view in MDA becomes the cleanup surface, and from that incident onward it is policed continuously. The Storm-2372 device-code-phishing cluster (active across 2024-2025) [Microsoft Security Blog: https://www.microsoft.com/en-us/security/blog/2025/02/13/storm-2372-conducts-device-code-phishing-campaign/] and Mint Sandstorm consent-phishing activity [Microsoft Security Blog: https://www.microsoft.com/en-us/security/blog/2023/04/18/nation-state-threat-actor-mint-sandstorm-refines-tradecraft-to-attack-high-value-targets/] both produced this exact rollout trigger in regulated FS. Microsoft threat-actor naming canonical reference: [Microsoft Learn: https://learn.microsoft.com/en-us/unified-secops/microsoft-threat-actor-naming].
...
- **Trigger:** internal IR identified a Storm-2372-style device-code-phishing wave targeting RM-coded mailboxes [Microsoft Security Blog: https://www.microsoft.com/en-us/security/blog/2025/02/13/storm-2372-conducts-device-code-phishing-campaign/]. Initial scoping found 73 users had consented to a "Microsoft Teams Connector" lookalike with `Mail.ReadWrite` + `MailboxSettings.ReadWrite` + `offline_access` over a 6-week window. Tenant had never run a baseline OAuth-grant inventory; no ban workflow existed
...
- **Outcome:** named app banned within 90 minutes of IR confirmation; mail-forwarding rules + delegate additions audited via Entra AuditLogs for the 73 users; 18 additional lookalike app IDs surfaced and banned over the next 4 weeks; control evidence pack delivered to BNM under the post-incident notification process [VERIFY current RMiT incident-notification clause + clock]; admin-consent workflow rolled out 90 days later with this policy as the cleanup layer underneath. Material on regulatory clocks is illustrative only and not legal/regulatory advice.

### Use case 2 — Merchant acquirer with multi-jurisdiction tenant, post-Mint Sandstorm consent-phishing wave

- **Org type:** payments processor operating across MY / SG / HK / TH; M365 E5; in-scope for PCI DSS + BNM RMiT + MAS TRM + HKMA SA-2
- **Trigger:** threat-intelligence brief from regional CERT identified Mint Sandstorm consent-phishing targeting payments-sector finance teams across ASEAN [Microsoft Security Blog: https://www.microsoft.com/en-us/security/blog/2023/04/18/nation-state-threat-actor-mint-sandstorm-refines-tradecraft-to-attack-high-value-targets/]. (Mint Sandstorm is the Microsoft canonical name; "TA453" is the Proofpoint taxonomy for an overlapping cluster — per [Microsoft Learn: https://learn.microsoft.com/en-us/unified-secops/microsoft-threat-actor-naming], the Microsoft canonical name should be used.) Board ask within 48 hours for "what is our exposure and what would we do if we found it". Tenant had only the built-in MDA anomaly policies running, all in alert-only
- **Scope:** ran the back-catalogue review across all four jurisdictional sub-tenants; built named-runbook for the ban workflow (decision gate, communication plan to integration owners, regulator-notification trigger per jurisdiction); enabled App Governance behavioural-anomaly detection (dynamic detection model, June 2025) in notify-only as the forward-looking layer
- **Outcome:** 12 lookalike apps surfaced and banned across the four sub-tenants; no confirmed compromise from Mint Sandstorm specifically, but the exercise surfaced 4 legitimately-consented ISVs with `Mail.ReadWrite` that the business had forgotten about — vendor-risk review triggered for all four; ban-runbook delivered to MAS / HKMA examiners on the next supervisory cycle as evidence of post-consent response capability. Material on supervisory expectations is illustrative only and not legal/regulatory advice.
...
- **Trigger:** examiner finding from the prudential supervisor citing lack of inventory and lifecycle management for third-party OAuth integrations in the cloud productivity stack. 90-day remediation clock [VERIFY against the supervisor's actual finding-resolution mechanic, not the citation here]. Material on supervisory clocks is illustrative only and not legal/regulatory advice.
...
| W6 | Tune anomaly-policy sensitivity; whitelist verified ISVs surfaced as FPs in W5 (lookalike publisher-name false matches, etc.). Begin App Governance behavioural-anomaly detection onboarding if licensed (notify-only) | First-tuning-pass anomaly policies; behavioural-anomaly baseline started |
...
|---|---|---|---|---|
| MDA | Microsoft Defender Portal → Cloud Apps → OAuth apps (or App Governance if add-on enabled). Built-in anomaly policies under Cloud Apps → Policies → Policy management → Threat detections | Day 1: manual review of every High-permission, low-community-use, unverified-publisher app; manually ban known-malicious; do NOT enable auto-revoke on built-in anomaly policy for at least 4 weeks while measuring FP rate; restrict user consent to verified-publisher apps at the Entra layer in tandem [Microsoft Learn: https://learn.microsoft.com/en-us/entra/identity-platform/publisher-verification-overview] | API-mode; covers M365 / Workspace / Salesforce delegated grants only | **Banning is tenant-wide and immediate, propagating on next token validation (CAE-aware apps revoke faster).** No staged rollout. Practitioners ban a flagged app at 11pm, wake up to dozens of broken integrations. **Application (client-credentials) OAuth grants are NOT in this view** — service-principal abuse is the dominant cloud-persistence pattern; **Practitioner inference:** Policy 2a covers ~30% of OAuth-grant abuse (practitioner observation; not Microsoft-documented); service-principal class needs App Governance behavioural-anomaly detection / Entra Workload Identities |
...
- **Advanced:** App Governance behavioural-anomaly detection (dynamic detection model, June 2025) live with measured FP rate; OAuth threshold policies tuned per-scope-tier; auto-revoke enabled selectively on highest-confidence signal classes; CASB OAuth signal correlated into Purview IRM as indicator; per-ISV DPA refresh on a documented cadence; SaaS-to-SaaS OAuth (Salesforce → Slack via inter-tenant consent) governed via Cross-Tenant Access Settings; Power Platform service-principal hygiene addressed via Power Platform Admin Center DLP as compensating control

## Control mappings

- **CIS Microsoft 365 Foundations Benchmark v5.0.0 (30 April 2025):**
  - **CIS 5.1.5.1 (L2)** — Ensure the admin consent workflow is enabled (primary upstream control)
  - **CIS 5.1.5.2 (L1)** — Ensure third-party integrated applications are not allowed / restrict user consent to verified-publisher apps
  - **CIS 5.1.2.2 (L2)** — Ensure that 'Restrict access to Microsoft 365 admin center' / equivalent administrative-consent governance is enabled
  - **CIS 2.4.3 (L2)** — Ensure Microsoft Defender for Cloud Apps is enabled (umbrella for OAuth-apps view)
- **Microsoft Secure Score** — Identity group ("Designate more than one global admin", "Restrict user consent to verified publishers") + Apps group ("Sanction or unsanction cloud apps"). Current Secure Score structure: Identity / Device / Apps / Data (four-group). Citations: [Microsoft Learn: https://learn.microsoft.com/en-us/defender-xdr/microsoft-secure-score] and [Microsoft Learn: https://learn.microsoft.com/en-us/defender-xdr/microsoft-secure-score-improvement-actions].
- **Zero Trust Identity pillar — Objective IV "Restrict user consent to applications"** [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/identity]
- **Cleanup workflow primary citation:** [Microsoft Learn: https://learn.microsoft.com/en-us/defender-office-365/detect-and-remediate-illicit-consent-grants]
- BNM RMiT clause(s): [BNM RMiT IAM + third-party app risk](../../06-compliance/malaysia/bnm-rmit.md) `[VERIFY against current edition]`. Material is illustrative only and not legal/regulatory advice.
...
| Steady-state | 3-8% | Quarterly attestation + continuous business-owner-register sync; App Governance behavioural-anomaly detection (if licensed) provides higher-confidence signal class |
...
| Power Platform connector consents (Premium connectors requiring `*.ReadWrite`) | Power Platform Admin Center DLP as the primary governance surface [Microsoft Learn: https://learn.microsoft.com/en-us/power-platform/admin/wp-data-loss-prevention]; MDA whitelist for the Power Platform service-principal pattern |
...
- Consent events catalogue user identity + grant scope + timestamp — workforce-monitoring data under PDPA / GDPR Art. 88 / equivalent. Material is illustrative only and not legal/regulatory advice.
...
- Antique Typhoon / Storm-2372-class supply-chain compromise: an approved-by-business ISV could turn malicious; documented kill-switch posture required as a control-design decision, not as a runbook footnote. (Antique Typhoon is the Microsoft canonical name for the cluster generally referenced in supply-chain-token-abuse discussion; the specific July 2023 token-forgery incident remains labelled Storm-0558. Source: [Microsoft Learn: https://learn.microsoft.com/en-us/unified-secops/microsoft-threat-actor-naming]; Storm-2372 reference: [Microsoft Security Blog: https://www.microsoft.com/en-us/security/blog/2025/02/13/storm-2372-conducts-device-code-phishing-campaign/].) Microsoft also documents historical OAuth abuse by Storm-1283 [Microsoft Security Blog: https://www.microsoft.com/en-us/security/blog/2023/12/12/threat-actors-misuse-oauth-applications-to-automate-financially-driven-attacks/].
...
- **Entra admin-consent workflow:** this policy is the compensating cleanup layer beneath admin-consent. Coupling document required — admin-consent governs new grants forward; MDA Policy 2a (internal numbering) governs the back-catalogue and provides the inventory + revocation workflow. Microsoft anchor: Identity Zero Trust Objective IV [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/identity] + [Microsoft Learn: https://learn.microsoft.com/en-us/defender-office-365/detect-and-remediate-illicit-consent-grants]
- **App Governance (where licensed):** Policy 2a (internal numbering) hands off the behavioural-anomaly detection (T1098.001 / T1136.003 / T1550.001 post-consent) to Policy 2b. Document the boundary; ensure no double-attestation overhead
- **Audit cycle:** quarterly OAuth-grant inventory + named-incident-investigation evidence feed the annual control-effectiveness review (under SOX 404 / NIS2 / DORA Art. 5-15 third-party-risk equivalent operational-risk regimes) `[VERIFY against the specific regime in scope]`. Material is illustrative only and not legal/regulatory advice.
...
- **Purview eDiscovery** (new eDiscovery experience in the Microsoft Purview portal; classic eDiscovery retired 2025-08-31) for the named-incident-investigation evidentiary record. Citation: [Microsoft Learn: https://learn.microsoft.com/en-us/purview/ediscovery-overview]
- **Purview IRM (Day 90, where deployed):** OAuth-anomaly signal can boost Insider Risk scoring when correlated with HR-departure or unusual-activity signals from the same user. Adaptive Protection wires IRM signal into DLP, DLM, and Conditional Access [Microsoft Learn: https://learn.microsoft.com/en-us/purview/insider-risk-management-adaptive-protection]
...
2. **"Enable auto-revoke on the built-in anomaly policies on day 1"** — Misleading-publisher-name and Uncommon-use generate 50-70% FP rate in W1 (practitioner-observed; not Microsoft-documented); auto-revoke at that FP rate breaks legitimate ISVs at scale within the first week
...
4. **"Treat Policy 2a as the OAuth-governance answer"** — covers delegated grants only. **Practitioner inference:** delegated-grant view covers ~30% of OAuth-abuse surface (practitioner observation; not Microsoft-documented). Service-principal / client-credentials grants (the dominant cloud-persistence pattern, T1098.001) are NOT in this view. Document the gap and the compensating control (App Governance + Entra Workload Identities Premium)
...
9. **"Ignore the Power Platform sub-class"** — Power Platform service-principal consents are under-surfaced in this view AND in App Governance. Heavy Power Platform tenants must additionally configure Power Platform Admin Center DLP [Microsoft Learn: https://learn.microsoft.com/en-us/power-platform/admin/wp-data-loss-prevention]; treating Policy 2a as covering Power Platform is the documented gap
...
- [OAuth blind spot](../../08-failure-modes/oauth-blind-spot.md) — application (client-credentials / app-only) grants are the dominant cloud-persistence pattern (**Practitioner inference:** ~70% of OAuth abuse; practitioner observation; not Microsoft-documented) and NOT in this policy's scope
- Personal-account OAuth (user signs into a third-party app with personal Gmail / personal MSA) — Entra never sees it
- First-time abuse of a never-before-anomalous app — behavioural-anomaly detection model has nothing to compare against; pair with Policy 2b once App Governance is licensed
- Power Platform service-principal consents — under-surfaced in App Governance; compensating control = Power Platform Admin Center DLP [Microsoft Learn: https://learn.microsoft.com/en-us/power-platform/admin/wp-data-loss-prevention]
...
- Midnight Blizzard / Antique Typhoon-class IdP-layer compromise — token-forgery at the IdP layer (e.g. the Storm-0558 July 2023 token-forgery incident, now associated with the Antique Typhoon cluster per Microsoft canonical naming) leaves no in-tenant detection in this view. Source: [Microsoft Learn: https://learn.microsoft.com/en-us/unified-secops/microsoft-threat-actor-naming]
