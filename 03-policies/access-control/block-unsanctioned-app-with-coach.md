...
> MDA playbook reference: [Policy 1 (Shadow IT discovery)](../../04-vendors/microsoft-defender-for-cloud-apps.md) for the discovery feed; enforcement layer via SWG/MDE. (Internal numbering — not Microsoft documentation.)

## Purpose

Block discovered SaaS apps tagged Unsanctioned (consumer-tier, high-risk, or unapproved business apps) and coach users toward the sanctioned alternative. Reduces uncontrolled SaaS adoption and data leakage to vendors outside the firm's risk-acceptance perimeter. Counters MITRE ATT&CK `T1567 Exfiltration Over Web Service` (parent technique retained for the broader shadow-app exfil class per X28 guidance) and `T1102 Web Service` C2 patterns; precondition for the broader Shadow IT control programme.

Microsoft architectural anchor: Application Zero Trust pillar — Initial Deployment Objective II ("Discover and control the use of shadow IT"). [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/applications]
...
|---|---|---|---|---|
| MDA | Microsoft Defender Portal → Cloud Apps → Cloud discovery → tag apps as Unsanctioned; pair with `Cloud Apps → Policies → Policy management → Shadow IT → App discovery policy` for auto-tagging | Tag = Unsanctioned; auto-tag policy with `Risk score ≤ 5` (Microsoft Cloud App Catalog risk-score scale is **0-10**, not 1-10, per [Microsoft Learn: https://learn.microsoft.com/en-us/defender-cloud-apps/risk-score]) + `Users per day > 10` thresholds; MDE Network Protection in **block mode** (not audit) | Endpoint enforcement requires Defender for Endpoint deployed with Network Protection in **block** mode. SWG block-script export is CDN-fronted — partial fail | Auto-unsanction propagates only to MDE-managed endpoints. BYOD-mobile + off-network + non-MDE = uncovered. "Generate block script for SWG" produces flat IP/domain list — modern GenAI vendors use rotating CDNs |
  ...
  criteria:
    sanctioned:
      requires:
        ...
        - "Risk score >= 7 (on Microsoft 0-10 scale) OR exception-approved"
        - "Reviewed by Privacy + Legal"
    tolerated:
      requires:
        - "Risk score >= 5 (on Microsoft 0-10 scale)"
        - "No regulated data in scope"
        - "Quarterly review for promotion or demotion"
    unsanctioned:
      auto_tag_policy:
        category: "any"
        risk_score: "<= 5"   # Microsoft Cloud App Catalog 0-10 scale
        users_per_day: "> 10"
...
- **BFSI:** BAA / DPA gate is non-negotiable for any app touching regulated data; Risk-score floor for sanction is typically higher (7+ on Microsoft's 0-10 scale); regulator sensitivity (BNM RMiT third-party / MAS Outsourcing) drives faster blocking of new high-risk discoveries
...
- **Advanced:** sanction-decision committee integrated with procurement workflow; new-vendor onboarding triggers sanction-review automatically; allowlist managed-as-code (Terraform-style + audit log); Microsoft Purview Adaptive Protection integration for risk-based per-user scoping (Adaptive Protection now wires into DLP, DLM, and Conditional Access — not just CA — per [Microsoft Learn: https://learn.microsoft.com/en-us/purview/insider-risk-management-adaptive-protection]); SaaS-admin-of-the-month rotation across BUs to maintain governance presence

## Control mappings

- **CIS Microsoft 365 Foundations Benchmark v5.0.0** (30 April 2025): CIS 1.3.4 (L1) — restrict non-admin app creation / user-shadow-IT discipline; CIS 5.1.2.2 (L2) — block unsanctioned cloud apps via MDA; CIS 2.4.3 (L2) — Defender for Cloud Apps as the umbrella detection / control surface
- **Microsoft Secure Score** (Apps group): improvement actions "Sanction or unsanction cloud apps" and "Block unsanctioned cloud apps". [Microsoft Learn: https://learn.microsoft.com/en-us/defender-xdr/microsoft-secure-score] [Microsoft Learn: https://learn.microsoft.com/en-us/defender-xdr/microsoft-secure-score-improvement-actions]
- **Microsoft Zero Trust — Application pillar:** Initial Deployment Objective II ("Discover and control the use of shadow IT"). [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/applications]
- BNM RMiT clause(s): [BNM RMiT third-party / outsourcing + cloud services](../../06-compliance/malaysia/bnm-rmit.md) `[VERIFY against current edition]` — illustrative only, not regulatory advice
...
Typical staffing: 0.2 FTE security analyst (sanction decisions + exception triage); committee participation ~2 hours / week across 5 members; help-desk impact ~10-20 tickets per week in month 1, declining. (Practitioner observation; not Microsoft-documented.)
...
5. **"Auto-unsanction by risk score alone"** — Microsoft / vendor opaque scoring overrides business reality; pair with human-in-the-loop review for any new auto-unsanction. **Practitioner inference:** Microsoft publishes the factor count ("90+ risk factors" per [Microsoft Learn: https://learn.microsoft.com/en-us/defender-cloud-apps/risk-score]) but does not publish the weighting — the score is opaque on the weighting side
