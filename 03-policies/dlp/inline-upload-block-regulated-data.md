...
> MDA playbook reference: [Policy 5 / 9](../../04-vendors/microsoft-defender-for-cloud-apps.md) (Block download of Confidential-labelled files; Block clipboard paste of sensitive data — same CAAC inspection mechanic on the upload leg). "Policy 5 / 9" is internal playbook numbering, not Microsoft documentation.

## Purpose

Prevent upload of files containing regulated data (PCI / PII / source / secrets) to sanctioned SaaS before the file lands. The pre-event prevention complement to API-mode DLP (post-event detect-and-remediate). Counters MITRE ATT&CK `T1567.002` (Exfiltration to Cloud Storage) on the upload leg. T1530 (Data from Cloud Storage Object) is intentionally excluded — T1530 is a Collection technique (adversary reading cloud storage), not Exfiltration; mapping a pre-upload inspection control to T1530 is a category error.
...
- **Scope:** Cards + Finance + Settlements BUs (~800 users); CAAC-onboarded apps = OneDrive + SharePoint Online + Box (Box used for partner-facing CHD exchange under contract); instance count (`minCount`) = 5; SIT = Credit Card Number at confidence level 85
...
- **Scope:** all employees tenant-wide for OneDrive + SharePoint Online (the M365 connector); phased CAAC onboarding for Workday + ServiceNow + Salesforce over 6 months; SITs = Malaysian IC Number (custom) + Passport Number (multi-jurisdiction) + Bank Account Number (custom); instance count (`minCount`) = 10 to suppress the high-volume FP class from staff-listing spreadsheets
...
- **Scope:** the acquirer's MDA estate became the target; the fintech's Netskope estate ran in parallel for 6 months; SIT alignment was the dominant work — Netskope EDM signatures had no direct Purview DCS equivalent for several custom classes; instance count (`minCount`) standardised at 5 across both estates as the interim baseline
...
| W2 | Per-SIT confidence level + instance count baseline. Run the chosen SITs in `Audit` mode across 30-day historical activity (via Purview DCS at-rest scan as proxy) to estimate FP volume | Baseline FP estimate by SIT; threshold candidates |
...
| W5-W6 | Flip pilot BU to `Block`. Monitor help-desk ticket volume daily. Iterate on coaching-message wording. Adjust per-SIT confidence level (typically raise from default 75 to 85) and instance count (typically raise from 1 to 5+) | Pilot BU blocked; ticket volume tracked; W6 review gate |
...
|---|---|---|---|---|
| MDA | Microsoft Defender Portal → Cloud Apps → Policies → Policy management → Conditional Access → Create Session policy. Plus the matching Entra Conditional Access policy with "Use Conditional Access App Control" | Session control type = Control file upload (with inspection); App = CAAC-onboarded list; Inspection method = Microsoft Purview Data Classification Service; SIT = Credit Card Number / U.S. SSN / EU passport / custom; **Confidence level ≥ 85 + instance count (`minCount`) ≥ 5** (not the default instance count of 1); Action = Block + custom user message; `Always apply if data cannot be scanned` = **document the trade-off** (false = leak via user-encrypted archive; true = block all unscannable including legitimate Zip / OneNote / proprietary formats) | Browser-only via CAAC. For Edge for Business users: direct in-browser protection — no `*.mcas.ms` URL rewrite [Microsoft Learn: https://learn.microsoft.com/en-us/defender-cloud-apps/proxy-intro-aad]. For non-Edge browsers: reverse-proxy via `*.mcas.ms` URL rewrite. Bilateral CAAC trap (CA policy + session policy required — either alone is a silent no-op). For non-Edge browsers, cert-pinned device profiles reject the `*.mcas.ms` chain and fail closed at TLS but look fail-open at the app layer. Document residual reverse-proxy dependency for non-Edge browsers; track Edge for Business adoption | DCS regex on SSN matches many 9-digit strings (employee IDs, order numbers) — high FP. The 100-character matching-content snippet is itself regulated content stored in Defender incident metadata. **Practitioner inference (field observation; not Microsoft-documented):** `window.parent.postMessage` cross-origin failure breaks Salesforce Lightning, ServiceNow Polaris, Workday Extend, Box file-preview, Atlassian Compass after CAAC rewrite via the non-Edge reverse-proxy path — regression-test every app before adding to scope. CAE-induced session re-establishment may bypass MDA policy re-evaluation — pair with Entra Token Protection |
...
policy:
  ...
  file_inspection:
    ...
    # Microsoft Purview SIT parameter naming: "confidence level" (`confidenceLevel`)
    # + "instance count" (`minCount` / `maxCount`). The older `match_accuracy`
    # / `minimum_unique_matches` synonyms are not the current Microsoft Learn names.
    sensitive_information_types:
      - id: "credit-card-number"
        confidenceLevel: 85          # raised from default 75; reduces format-collision FPs
        minCount: 5                  # instance count; NOT default of 1
      ...
      - id: "us-ssn"
        confidenceLevel: 85
        minCount: 10                 # high threshold — SSN regex collides with employee IDs
    fail_closed_on_unscannable: false   # documented decision: legitimate Zip/OneNote not blocked
                                        # compensating control = at-rest scan via internal API-mode policy
  governance:
    action: Block
    user_message: |
...
The `confidenceLevel: 85` + `minCount: 5` (instance count) combination is the typical post-tuning baseline for the Credit Card Number SIT. The default `confidenceLevel: 75` + instance count of 1 produces W1 FP rates above 50% and is not viable for production. The fail-closed-on-unscannable = false decision is the single most argued point — document the compensating control (at-rest scan via the [auto-label PCI policy](auto-label-pci-data.md)) explicitly in the control-design record.
...
- **Immature deployment** looks like: one session policy tenant-wide, default SIT confidence level (~75), instance count = 1, action = Block on day one, no audit-mode pilot, no per-SIT tuning, no documented exception path, no coaching message (or generic "blocked by policy" message). W1 FP rate >50%; help-desk overwhelmed; programme rolls back to audit-only within 2 weeks; trust in inline DLP damaged for 12+ months.

- **Mature deployment** looks like: per-BU policy scoping with phased rollout completed; per-SIT confidence level and instance counts tuned (85 / 5+ as typical baseline); documented audit-mode pilot evidence on file; coaching message reviewed quarterly; exception process named with SLA; integration with auto-label PCI policy for at-rest compensating control; integration with Sentinel for the three-record audit trail; FP rate <10% steady state; quarterly tuning cadence.

- **Advanced deployment** looks like: per-SIT confidence level tuned per BU (Cards BU uses higher CHD threshold than Risk BU); custom SITs for the firm's own data formats (internal BIN ranges, internal reference numbers, internal product codenames); EDM-equivalent signatures where Purview supports `[VERIFY current EDM status in Purview]`; bilateral CAAC pair version-pinned in source control with documented change-management; coaching message branches by SIT class (CHD message differs from secrets message); CAE + Entra Token Protection in place; Purview IRM signal-boost on repeated block events feeding the insider-risk programme.

## Control mappings

Regulatory material below is illustrative — not legal or regulatory advice.

- **Microsoft Zero Trust — Data pillar (Objective IV "Protect sensitive data via encryption, access restrictions, and visual markings"):** the documented home for inline DLP and the three governance actions (remove permissions, quarantine, apply labels) [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/data]
- **Microsoft Zero Trust — Application pillar (Objective III "Apply analytics to monitor for abnormal behaviour"):** the documented home for CAAC session control [Microsoft Learn: https://learn.microsoft.com/en-us/security/zero-trust/deploy/applications]
- **CIS Microsoft 365 Foundations Benchmark v5.0.0 (30 April 2025):**
  - **CIS 3.2.1 (L1)** "Ensure DLP policies are enabled" — policy-intent map
  - **CIS 3.2.2 (L1)** "Ensure DLP policies are enabled for Microsoft Teams" — policy-intent map
  - **CIS 3.3.1 (L1)** "Ensure SharePoint Online Information Protection policies are set up and used" — policy-intent map
  - **CIS 2.4.3 (L2)** "Ensure Microsoft Defender for Cloud Apps is enabled and configured" — CAAC inspection mechanism map (umbrella control for MDA-dependent capabilities)
- **Microsoft Secure Score:** improvement actions under the **Data** group (DLP rule coverage) and the **Apps** group (Defender for Cloud Apps onboarding and session-policy posture) — current four-group structure (Identity / Device / Apps / Data) [Microsoft Learn: https://learn.microsoft.com/en-us/defender-xdr/microsoft-secure-score] [Microsoft Learn: https://learn.microsoft.com/en-us/defender-xdr/microsoft-secure-score-improvement-actions]
...
|---|---|---|
| W1 | 45-65% | Default SIT confidence level (~75) + instance count (`minCount`) = 1; SSN regex on employee IDs; CCN regex on order numbers; test-data files in engineering uploads |
| W4 | 20-30% | After per-SIT confidence-level raise (75 → 85) + instance count raise (1 → 5); after first-pass exclusion list for known reference-number formats |
...
| Scenario | Mitigation |
|---|---|
| 9-digit employee ID matches U.S. SSN regex | Raise SSN confidence level to 90; require co-occurring context tokens ("SSN:" / "Social Security"); exclude HR-system-derived upload paths |
...
| Spreadsheets with numeric ID columns matching multiple SIT regexes at once | Raise instance count (`minCount`); require context tokens; consider per-column DCS context inspection where supported |
...
| `postMessage` cross-origin failure on Salesforce Lightning / ServiceNow Polaris after CAAC rewrite blocks legitimate in-app file attach (practitioner observation; not Microsoft-documented) | Pre-CAAC compat test required; if breakage confirmed, remove app from CAAC scope until vendor fix; document as coverage gap |
...
- Cross-border processing consideration: the inspection (DCS) happens at the tenant primary-data region; for MY / SG / HK tenants the Japan East primary-data region `[VERIFY against current Microsoft Cloud regions page]` materially improves the cross-border story [Microsoft Learn: https://learn.microsoft.com/en-us/microsoft-365/enterprise/o365-data-locations]
...
- **M&A IT integration playbook:** the SIT inventory + per-SIT confidence-level + instance-count config becomes the standardisation target when integrating an acquired entity's DLP estate; document the standard as a reusable artefact
- **Endpoint DLP + Browser DLP via Edge for Business as primary Microsoft-documented controls:** Microsoft positions Endpoint DLP and Edge for Business Browser DLP as the primary documented controls for the upload / paste-block use case against third-party AI sites and unsanctioned cloud apps. CAAC session-policy inline-upload-block (this policy) covers the sanctioned-app inspection lane; Endpoint DLP covers the device-egress lane; Edge for Business Browser DLP covers the browser-paste lane. Run them as a stack, not as alternatives [Microsoft Learn: https://learn.microsoft.com/en-us/purview/dlp-browser-dlp-learn] [Microsoft Learn: https://learn.microsoft.com/en-us/purview/dlp-create-policy-prevent-cloud-sharing-from-edge-biz] [Microsoft Learn: https://learn.microsoft.com/en-us/purview/endpoint-dlp-learn-about]
...
2. **"Use the vendor's default confidence level and instance count"** — default 75% confidence + instance count of 1 produces W1 FP rates above 50%. The defaults are vendor-marketing-optimised, not production-ready
...
8. **"Ignore the postMessage breakage class"** — Salesforce Lightning, ServiceNow Polaris, Workday Extend, Box file-preview, Atlassian Compass break after CAAC reverse-proxy rewrite via `window.parent.postMessage` cross-origin failure (practitioner observation; not Microsoft-documented). Pre-CAAC compat test is mandatory; rolling out the policy and then finding out an app is broken creates business-critical workflow outages. Edge for Business users on the direct in-browser protection path are unaffected [Microsoft Learn: https://learn.microsoft.com/en-us/defender-cloud-apps/proxy-intro-aad]
...
- Cert-pinned native apps refuse the `*.mcas.ms` cert chain on the non-Edge reverse-proxy path — fail closed at TLS but appear fail-open at app layer
...
- `postMessage` cross-origin failure on SaaS apps using cross-frame messaging in their UI (Salesforce Lightning, ServiceNow Polaris, Workday Extend, Box file-preview, Atlassian Compass) — breaks legitimate in-app functionality after CAAC reverse-proxy rewrite (practitioner observation; not Microsoft-documented). Edge for Business users on the direct in-browser protection path are unaffected [Microsoft Learn: https://learn.microsoft.com/en-us/defender-cloud-apps/proxy-intro-aad]
