...
02-capabilities/     Capability catalogue — one entry per capability (Shadow IT discovery, OAuth-app governance, inline DLP, anomaly detection, etc.), schema-driven, with deployment-mode dependencies and vendor coverage notes
03-policies/         Policy catalogue — one entry per policy archetype (block-unsanctioned-app, DLP-for-sanctioned-SaaS, anomalous-download, OAuth-grant-revocation, share-link-quarantine, etc.) with use-case, controls mapped, false-positive risk, ops cost
...
| **Capability vs Policy separation** | Strict — capabilities in `02-`, policies in `03-`, never inline together | The user's framing conflated them; the discipline forces clarity |
